...
int parse_field(field f) {
	return 8 * (f.y - 1) + f.x - 'a';
}

void setup_board(piece board[64]) {
	board[0] = white|rook;
...
int is_move_legal(chess_game game, move m) {
...
move_result make_move(chess_game *game, move m) {
